...
class Generator(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        #print('x:', x.shape, 'x_stage1:', x_stage1.shape, 'mask:', mask.shape, 'self.cnum_in', self.cnum_in)
        x = x_stage1*mask + xin[:,0:1,:,:]*(1.-mask)  #-2 -1 ? x = x_stage1*mask + xin[:, :self.cnum_in-1]*(1.-mask)
        #print('here', x.shape)
        # get refined result
